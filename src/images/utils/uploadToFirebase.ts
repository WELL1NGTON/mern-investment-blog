...
export default async function (
  ...
  try {
    ...
    return { url, firebaseStorageDownloadTokens: token, firebaseFileName: uploadedImage[0].name, bucket: uploadedImage[0].bucket.name };
  } catch (e) {
    throw e;
  } finally {
    deleteFile(path);
...
const generateNewCompressedFile = async (imageCompressOptions: IImageCompressOptions) => {
  ...
  const newPath = generateNewFilePathName(imageCompressOptions.file.path, imageCompressOptions.format);
...
const deleteFile = async (path: string) => {
  ...
  await fs.access(path, (err) => {
    if (!err) {
      ...
      // Delete the file
      fs.unlink(path, (err) => {
        if (err) throw new AppError(err.message, StatusCodes.INTERNAL_SERVER_ERROR);
      });
    } else {
